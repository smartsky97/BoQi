...
function showzc(){
	...
	$("#che").html("请输入验证码").css("color","#CC6666");
	$("#loginpages").hide();
...
function checkzccode() {
	...
	$.post("doLoginServlet?d=" + new Date(), {op : "checkcode",zccode:zccode}, function(data) {
		if (parseInt(data) < 1) {
			$("#zccodep").html("验证码有误，请重新输入...");
			$("#zccode").val("");
		} else {
			$("#zccodep").html("验证码正确...");
		}
...
function changeVilidateCode(obj){
	...
	obj.src="js/valiCodeImg.jsp?d="+timenow;
}
...
function userlogin(){
	...
	$.post("userServlet?d="+new Date(),{op:"userLogin2",uname:uname,pwd:pwd},function(data){
...
function send11(){
	var ema=$("#zcemail").val();
	$.post("sendYan",{ema:$.trim(ema)},function(data){
		var get = data;
		yan = data;
		//alert(get+"<-->haha");
	});
...
//判断输入的验证码是否正确
function checkYan(){
	var getin = $("#che").val().toLowerCase();
	if(typeof(yan)!="undefined" && getin==yan.toLowerCase()){
		a5=1;
		$("#checkyanzhengma").html("验证码正确...").css("color", "#0F0");
	}else{
		a5=0;
		$("#che").val("");
        $("#checkyanzhengma").html("验证码有误...").css("color", "#F00");
	}
}
...
function useregister(){
	var email=$("#email").val();
	var registerpwd=$("#pwd").val();
	var uname=$("#uname").val();
	$.post("../userServlet",{op:"addUserinfo",email:email,pwd:pwd,registerpwd:registerpwd,uname:uname},function(data){
		if(data==1 && a1==1 && a2==1 && a3==1 && a4==1 && a5==1 ){
			alert("注册成功...");
